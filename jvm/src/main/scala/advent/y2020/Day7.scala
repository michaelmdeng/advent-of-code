...
object Day7 extends SafeDayRunner[String, Int, Int] {
    ...
    val contentsGraph =
      Graph.from(contentsByColor.keys, contentsByColor.flatMap {
        case (color, colorContents) =>
          colorContents.map {
            case (_, c) => GraphEdge.DiEdge(color, c)
          }
      })
  ...
  private def sumContainingMemo: mutable.Map[String, Int] = mutable.Map()

  private def sumContaining(
    contentsByColor: Map[String, Set[(Int, String)]]
  ): Int =
    sumContainingHelper(contentsByColor, SHINY_GOLD) - 1 // don't count the bag itself

  private def sumContainingHelper(
    contentsByColor: Map[String, Set[(Int, String)]],
    curr: String
  ): Int = {
    if (sumContainingMemo.contains(curr)) {
      sumContainingMemo(curr)
    } else {
      val out = 1 + contentsByColor(curr).toSeq.map {
        case (amt, next) => {
          amt * sumContainingHelper(contentsByColor, next)
        }
      }.sum

      sumContainingMemo(curr) = out
      out
    }
  }
