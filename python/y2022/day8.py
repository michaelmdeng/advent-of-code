    ...
    def run_part_1(self, input):
        ...
        for row_idx, row in enumerate(tree_grid):
            for col_idx, tree_height in enumerate(row):
                ...
                if len(above_heights) == 0 or all(
                    [height < tree_height for height in above_heights]
                ):
                ...
                if len(below_heights) == 0 or all(
                    [height < tree_height for height in below_heights]
                ):
                ...
                if len(to_left_heights) == 0 or all(
                    [height < tree_height for height in to_left_heights]
                ):
                    left_visible[row_idx][col_idx] = True

                to_right_col_idx = range(col_idx + 1, len(tree_grid[0]))
                to_right_heights = [
                    tree_grid[row_idx][cidx] for cidx in to_right_col_idx
                ]
                if len(to_right_heights) == 0 or all(
                    [height < tree_height for height in to_right_heights]
                ):
        ...
        for row_idx in range(0, len(tree_grid)):
            for col_idx in range(0, len(tree_grid[0])):
                visible[row_idx][col_idx] = (
                    top_visible[row_idx][col_idx]
                    or bottom_visible[row_idx][col_idx]
                    or left_visible[row_idx][col_idx]
                    or right_visible[row_idx][col_idx]
                )
    ...
    def run_part_2(self, input):
        ...
        for row_idx in range(1, len(tree_grid) - 1):
            for col_idx in range(1, len(tree_grid[0]) - 1):
                scores[row_idx][col_idx] = self.scenic_score(
                    row_idx, col_idx, tree_grid
                )
